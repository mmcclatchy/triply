...
const InitMap = ({}) => {
  ...
  const displayedSuggestions = useSelector(
  ...
  useEffect(() => {
    if (tripComplete) {
      ...
      const matches = document.querySelectorAll(
        'div.gm-style-iw, div.gm-style-iw-c, div.gm-style-iw-t'
      );
  ...
  return (
    <>
      {!displayedSuggestions?.restaurants ||
      !displayedSuggestions?.gasStations ? (
        <GoogleMap zoom={12} id={'newMap'} center={center}>
          {directions ? <DirectionsRenderer directions={directions} /> : null}
        </GoogleMap>
      ) : (
        <GoogleMap id='newMap' zoom={zoom} center={center}>
